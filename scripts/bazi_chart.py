"""
八字排盘脚本
使用方法：修改下方配置区，运行脚本，输出排盘 markdown。
"""

import subprocess
subprocess.run(['pip', 'install', 'lunar_python', '--quiet'], check=False)
...
def equation_of_time_minutes(year, month, day):
    ...
    返回当日的均时差（分钟），一年内在约 -14 ~ +16 分钟之间变化。
    11 月达到正向最大值约 +16，2 月达到负向最大值约 -14。
    """
...
MINUTE = 0
GENDER = '男'      # '男' 或 '女'
LOCATION = '北京'  # 出生城市，没有就填 None
LONGITUDE = None   # 直接指定经度（优先级高于 LOCATION），没有就 None
...
if LONGITUDE is not None:
    ...
    # 出生地不在表中：经度时差不应用，但均时差仍生效
    location_used = f"{LOCATION}（不在表中，仅应用均时差）"

# 没有提供任何位置信息时，连均时差都不应用（默认北京时间）
if LONGITUDE is None and not (LOCATION and LOCATION in CITY_LONGITUDE) and not LOCATION:
    eot_correction = 0.0
